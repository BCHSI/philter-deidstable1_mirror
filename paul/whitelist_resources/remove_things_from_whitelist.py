...
def get_names(files, locations):
    ...
    for i in range(len(files)): # go through each file
        ...
        file = files***REMOVED***i***REMOVED*** # the file name
        loc = locations***REMOVED***i***REMOVED*** # the separator (eg. 'whitespace' in a txt file), locator (eg. a column in a csv file)
        names_temp = ***REMOVED******REMOVED*** # the names from the current file to be added to the big list
        extension = file.split(".")***REMOVED***-1***REMOVED***
        ...
        with open(file) as fin:
            ...
            if extension == "txt": # for txt files
                ...
                if "ss_firstnames" in file: # for special case "ss_firstnames.txt"
                    lines = fin.readlines()
                    for line in lines:
                        names_temp.append(line.split(loc)***REMOVED***0***REMOVED***)
                ...
                if loc >= len(df.columns) or loc < 0: # if the column doesn't exist in the csv file
                    ...
                    names_temp = df.iloc***REMOVED***:,loc***REMOVED***
            ...
            for name in names_temp: # go through each name from the current file
                ...
                if " " in n: # if name comprises of two or more words, add to dict and skip
                    if n in two_word_names.values():
                        if file not in two_word_names***REMOVED***n***REMOVED***:
                            two_word_names***REMOVED***n***REMOVED***.extend(***REMOVED***file***REMOVED***)
                    else:
                        two_word_names***REMOVED***n***REMOVED*** = ***REMOVED***file***REMOVED***

                else: # if name is one word, add to dict
                    if n in names.values():
                        if file not in names***REMOVED***n***REMOVED***:
                            names***REMOVED***n***REMOVED***.extend(***REMOVED***file***REMOVED***)
                    else:
                        names***REMOVED***n***REMOVED*** = ***REMOVED***file***REMOVED***
...
def check_wl(names, wl_in, con): # con is confirmation
    ...
    for name in names: # go through each name
        if name in wl_dict: # check if name exists in whitelist
            names_in_both***REMOVED***name***REMOVED*** = names***REMOVED***name***REMOVED***
    ...
    if names_exist: # if there are names in the whitelist
        ...
        for name in names_in_both: # go through each name in the whitelist
            ...
            if ans == "y":
                removed_values***REMOVED***name***REMOVED*** = names_in_both***REMOVED***name***REMOVED***
            elif ans == "n":
                kept_values***REMOVED***name***REMOVED*** = names_in_both***REMOVED***name***REMOVED***
        ...
        for name in removed_values:
            print(name, removed_values***REMOVED***name***REMOVED***)
        ...
        if con:
            ...
            for name in kept_values:
                print(name, kept_values***REMOVED***name***REMOVED***)
        ...
        if ans == "y":
            for name in removed_values:
                del wl_dict***REMOVED***name***REMOVED***

            new_wl = "remove_things_from_whitelist_out/"+wl_in.split("/")***REMOVED***-1***REMOVED***
    ...
    if names_in_both is not None:
        with open("remove_things_from_whitelist_out/names_in_whitelist.txt", "w") as fout: # output the names in the whitelist
            for key in names_in_both:
                s = str(key)+"    "+str(names_in_both***REMOVED***key***REMOVED***)+"\n"
                fout.write(s)
