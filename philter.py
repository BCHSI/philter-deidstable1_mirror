...
def main():
    ...
    if args.multi == True:
        ...
        if os.path.isdir(finpath):
            if args.recursive:
                results = [pool.apply_async(filter_task, (f,)+(whitelist, foutpath, key_name, NumPhilter)) for f in glob.glob   (finpath+"/**/*.txt", recursive=True)]
            else:
                results = [pool.apply_async(filter_task, (f,)+(whitelist, foutpath, key_name, NumPhilter)) for f in glob.glob   (finpath+"/*.txt")]
        else:
            results = [pool.apply_async(filter_task, (f,)+(whitelist, foutpath, key_name, NumPhilter)) for f in glob.glob(  finpath)]
    else:
