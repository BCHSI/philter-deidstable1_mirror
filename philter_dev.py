            ...
            for i in range(len(sent_tag***REMOVED***0***REMOVED***)):
                ...
                if word_output not in string.punctuation:
                    ...
                    try:
                        ...
                        if re.findall(r'^***REMOVED***A-Za-z***REMOVED***+$', word_check) is not None:
...
def main():
    ...
    if os.path.isdir(finpath):
        if args.recursive:
            results = ***REMOVED***pool.apply_async(filter_task, (f,)+(whitelist, blacklist, foutpath, key_name)) 
                      for f in glob.glob(finpath+"/**/*.txt", recursive=True)***REMOVED***
        else:
            results = ***REMOVED***pool.apply_async(filter_task, (f,)+(whitelist, blacklist, foutpath, key_name)) 
                      for f in glob.glob(finpath+"/*.txt")***REMOVED***
    else:
