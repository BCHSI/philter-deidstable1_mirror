...
def parse_xml_files(directory,output_directory,philter_or_i2b2):
	...
	date_shift_log.columns = ["Filename", "Input Date", "Shifted Date","Time Delta"]
...
def shift_dates(filename_dates,filename,xmlstr,date,date_shift_log,verbose):
    ...
    if dt !=None:
      ...
      if strict_parse == None or strict_parse.year != dt.year:
          ...
          date_shift_log = date_shift_log.append([(filename,date,output_shifted_date,time_delta_str)])
        ...
        date_shift_log = date_shift_log.append([(filename,date,"cannot parse date",time_delta_str)])
