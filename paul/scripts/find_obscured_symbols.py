...
def pull_paths(inputfile):
    ...
    with open(inputfile) as fin:
        l = re.findall('^\.(***REMOVED***^\n***REMOVED***+?):', fin.read(), re.MULTILINE)
        for item in l:
            temp.append(item)
    return temp
...
def pull_lines(inputfile):
    temp = ***REMOVED******REMOVED***
    with open(inputfile) as fin:
        l = re.findall('^\.***REMOVED***^\n***REMOVED***+?\.txt\:(***REMOVED***0-9***REMOVED***+)\:', fin.read(), re.MULTILINE)
        for item in l:
            temp.append(item)
    return temp
...
def setup_names(paths, names):
    return {paths***REMOVED***i***REMOVED***: names***REMOVED***i***REMOVED*** for i in range(len(paths))}
...
def setup_lines(names, lines):
    ...
    for i in range(len(names)):
        name = names***REMOVED***i***REMOVED***
        line = lines***REMOVED***i***REMOVED***
        if name in temp:
            temp***REMOVED***name***REMOVED***.append(line)
        else:
            temp***REMOVED***name***REMOVED*** = ***REMOVED***line***REMOVED***
    return temp
...
def copy(sources, destpath):
    ...
    for s in sources:
        name = s.split("/")***REMOVED***-1***REMOVED***
        ...
        shutil.copy(sources***REMOVED***s***REMOVED***, dest)
...
def compare(notesdir, resultsdir, path_name, name_lines, symbols):
    ...
    copy = {}
    rpaths = ***REMOVED******REMOVED***
    rout = ***REMOVED******REMOVED***
    rnames = ***REMOVED******REMOVED***
    rsmbs = ***REMOVED******REMOVED***
    print("\n\n==============================================================")
    ...
    for filepath in path_name:
        ...
        name = path_name***REMOVED***filepath***REMOVED***
        ...
        for line in name_lines***REMOVED***name***REMOVED***:
            og_line = foriginal***REMOVED***int(line)-1***REMOVED***
            ph_line = fphilter***REMOVED***int(line)-1***REMOVED***

            # check lines for symbols
            for smb in symbols:
                term = "***REMOVED***^A-Za-z***REMOVED***" + re.escape(smb) + "***REMOVED***^A-Za-z***REMOVED***"
                if re.search(term, og_line):
                    if not re.search(term, ph_line):

                        # output
                        s = "'" + smb + "' was obscured in '" + path_name***REMOVED***filepath***REMOVED*** + "' on line '" + line + "' : \"" + ph_line + "\""
                        ...
                        copy***REMOVED***notesdir+name***REMOVED*** = resultsdir+name
