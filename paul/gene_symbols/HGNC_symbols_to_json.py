...
with open("HGNC_gene_names.txt") as fin:
    ...
    alias_sym = re.findall('(?:Approved|Entry Withdrawn)\t\S*\t***REMOVED***\S ***REMOVED****\t(\S+)', fin.read())
    previous_sym = re.findall('(?:Approved|Entry Withdrawn)\t\S*\t***REMOVED***\S ***REMOVED****\t\S*\t***REMOVED***\S ***REMOVED****\t***REMOVED***\S ***REMOVED****\t(\S+)', fin.read())
...
for item in approved_sym: # approved symbols
    symbols***REMOVED***item***REMOVED*** = 1
