...
#pattern_adjacent_names = re.compile(r"((***REMOVED***A-Z***REMOVED***\'?***REMOVED***A-Z***REMOVED***?***REMOVED***\-aA-zZ***REMOVED***+)(\s|\s\,\s)\*\*PHIName\*\*|\*\*PHIName\*\*(\s|\s\,\s)(***REMOVED***A-Z***REMOVED***\'?***REMOVED***A-Z***REMOVED***?***REMOVED***\-aA-zZ***REMOVED***+))")
...
def main():
    ...
    if os.path.isdir(finpath):
        if args.recursive:
            results = ***REMOVED***pool.apply_async(filter_task, (f,)+(whitelist, blacklist, foutpath, key_name)) for f in glob.glob   (finpath+"/**/*.txt", recursive=True)***REMOVED***
        else:
            results = ***REMOVED***pool.apply_async(filter_task, (f,)+(whitelist, blacklist, foutpath, key_name)) for f in glob.glob   (finpath+"/*.txt")***REMOVED***
    else:
        results = ***REMOVED***pool.apply_async(filter_task, (f,)+(whitelist, blacklist, foutpath, key_name)) for f in glob.glob(  finpath)***REMOVED***
    try:
