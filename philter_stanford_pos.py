...
class Philter:
    ...
    def __init__(self, config):
        ...
        if "stanford_pos_tagger" in config:
            if not os.path.exists(config["stanford_pos_tagger"]["model"]) and config["stanford_pos_tagger"]["download"] == False:
                raise Exception("Filepath does not exist", config["stanford_pos_tagger"]["model"])
            else:
                #download the ner data
                process = subprocess.Popen("cd generate_dataset && ./download_ner.sh".split(), stdout=subprocess.PIPE)
                output, error = process.communicate()
            self.stanford_pos_tagger_model = config["stanford_pos_tagger"]["model"]
            if not os.path.exists(config["stanford_pos_tagger"]["jar"]):
                raise Exception("Filepath does not exist", config["stanford_pos_tagger"]["jar"])
            self.stanford_pos_tagger_jar = config["stanford_pos_tagger"]["jar"]
                #we lazy load our tagger only if there's a corresponding pattern


        self.stanford_ner_tagger = None

        self.stanford_pos_tagger = None
    ...
    def map_pos(self, filename="", text="", pattern_index=-1, pre_process= r"[^a-zA-Z0-9\.]"):
        ...
        #load and create a POS tagger if it doesn't exist
        if self.stanford_pos_tagger == None:
            model_path = self.stanford_pos_tagger_model #'/usr/local/stanford-ner/classifiers/english.all.3class.distsim.crf.ser.gz'
            jar_path = self.stanford_pos_tagger_jar #'/usr/local/stanford-ner/stanford-ner.jar'     
            self.stanford_pos_tagger = StanfordPOSTagger(model_path,jar_path)
        ...
        pos_list = self.stanford_pos_tagger.tag(cleaned)
        # if filename == './data/i2b2_notes/160-03.txt':
        #     print(pos_list)
        start_coordinate = 0
        for tup in pos_list:
    ...
    def eval(self,
        ...
        for fn in fn_tags:
            ...
            for subfile in file_dict:
                ...
                filename = fn.split('/')[-1]
                fn_tags_condensed_context[key_name] = [word, phi_tag, pos_tag, fn_context, filename]
                context_counter += 1
        ...
        for fp in fp_tags:
            ...
            for subfile in file_dict:
                ...
                key_name = "uniq" + str(context_counter)
                filename = fp.split('/')[-1]
                fp_tags_condensed_context[key_name] = [word, pos_tag, fp_context, filename]
                context_counter += 1
        ...
        with open(fn_tags_context, "w") as fn_file:
            ...
            for key in fn_tags_condensed_context:
                ...
                fn_file.write(key + "|" + current_list[0] + "|" + current_list[1] + "|" + current_list[2] + "|" + current_list[3] + "|" + current_list[4]+"\n")
        ...
        with open(fp_tags_context, "w") as fp_file:
            ...
            for key in fp_tags_condensed_context:
                ...
                fp_file.write(key + "|" + current_list[0] + "|" + current_list[1]  + "|" +  current_list[2] + "|" + current_list[3]+"\n")
