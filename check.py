...
def annotating(note, phifile):
    ...
    phifile = ***REMOVED***word for word in phifile if '**PHI' not in word***REMOVED***
    ...
    for sent in note:
        ...
        for j in range(len(word)):
            if word***REMOVED***j***REMOVED*** in philter_phi:
                sent_list.append(***REMOVED***word***REMOVED***j***REMOVED***, '1', j + 1***REMOVED***)
                temp_sent += '**PHI-'+word***REMOVED***j***REMOVED***+'** '
            else:
                sent_list.append(***REMOVED***word***REMOVED***j***REMOVED***, '0', j + 1***REMOVED***)
                temp_sent += word***REMOVED***j***REMOVED***+' '
...
Each sublist contains 2 elements: ***REMOVED***word_from_original_text, phi_label***REMOVED***
    ...
    if anno_mode == False:
        ...
        for f in glob.glob(os.path.join(finpath, '*.txt.done')):
            head,tail = os.path.split(''.join(f.split('.done')***REMOVED***:-1***REMOVED***))
            done_set.add(tail)
        doing_set = set()
        for f in glob.glob(os.path.join(finpath, '*.txt.doing')):
            head,tail = os.path.split(''.join(f.split('.doing')***REMOVED***:-1***REMOVED***))
            doing_set.add(tail)
        phi_set = set()
        for f in glob.glob(os.path.join(phipath, '*_phi_reduced.txt')):
            ...
            phi_set.add(tail)
        to_do_set = annotation_set-done_set-doing_set
        if len(to_do_set) > 0:
        ...
        with open(to_do, encoding='utf-8', errors='ignore') as fin:
            note = fin.read()
        doing_path = os.path.join(finpath, to_do + '.doing')
        done_path =  os.path.join(finpath, to_do + '.done')
